...
built from the *Fragrance ecommerce redesign* handoff (`Kilau Bali.dc.html`).

Kilau Bali pours each scent in small numbers. Customers **commit** to a batch —
...
> **Aesthetic** — Modern Apothecary × High-End Minimalist, on paper. Gallery
> white (`#ffffff`) with a warm surface (`#faf7f2`), near-black ink (`#14120e`),
> and the sunburst golds from the Kilau Bali mark: antique gold (`#8a6215`) for
> accent type, leaf gold (`#c8a063`) for fills, and `#b8873c` for gold rules.
> Cormorant Garamond headings, Hanken Grotesk body, Space Mono data.
>
> **Contrast** — every text tone in the palette is chosen against the surface it
> lands on: body copy runs 4.9:1 or better, accent gold 5.5:1, ink on a gold
> button 7.7:1, and control borders clear 3:1. `npm run audit:contrast` measures
> this against the rendered page rather than the stylesheet — see
...
- **Homepage** — hero (*Wear it. Live it. Take it with you.*), the four ranges
  ...
  horizontal scent cards, and the Kilau Drive / Kilau Ritual banners.
...
- **Hero** — *What does your personality smell like?* over drifting vapour, a
  molecule lattice and a paper horizon, with an animated miniature
...
behind the paper scrim). Add these to match the comp exactly:

| Path | Used by |
| --- | --- |
| `assets/hero-lineup.jpg` | Homepage hero — 50 ml, 30 ml, discovery, car diffuser, wash, moisturiser |
| `assets/10-ml-bottles-Remix-1.jpg`, `30 ml bottle.jpeg`, `car-freshner.jpg`, `body and sets.jpg` | "Choose your Kilau" tiles (in place) |
...
> **Photography still carries the previous house's wordmark.** The stock shots
> in `public/assets/` were shot for the original storefront and several of them
> have that name printed on the bottle or the box. The paper scrim over each
> photograph (0.88–0.97 white where copy sits) plays it well down, but the only
> real fix is new photography: drop replacements in at the same paths and
...
- **AI fragrance conception** (admin) — type a reference fragrance and Claude proposes
  the Kilau Bali name (**Brand Conception**), writes tagline, story and packaging
...
quietly breaks: an alpha that reads at 4.9:1 as cream on black is 2.6:1 as ink
on white, and a scrim that made photography safe for light type makes it unsafe
for dark type. So the check does not read the stylesheet — it reads the rendered
page.
...
all text transparent, screenshots the page, and measures the element's colour
against **the pixels actually behind its glyphs** — which is the only way to
judge type sitting on photography or on an animated gradient. Text painted over
by something else (anything scrolled under the sticky header) is skipped rather
...
└── components/
    ├── Header.tsx  Hero.tsx  ChooseKilau.tsx  FindYourScent.tsx  MoodShop.tsx  RangeBanners.tsx
