...
export function route(
  ...
  // Most of these routes are the checkout path, so that is the default. A
  // route that is not — the supplier queue, the setup report — says so, since
  // "Checkout could not start" on the admin console is a misleading error.
  failure = "Checkout could not start",
) {
  return async (req: any, res: any) => {
    try {
      ...
      json(res, 500, { error: failure, detail: `${name}: ${detail}` });
    }
  };
}
