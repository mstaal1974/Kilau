// Vercel serverless function — Kilau Bali AI fragrance conception.
//
...
const SCHEMA = {
  ...
  properties: {
    name: {
      type: "string",
      description:
        "The Kilau Bali house name for this scent. Two evocative words, Title Case, no brand names, no trademarked terms, not already in the house catalogue.",
    },
    alternates: {
      type: "array",
...
const SYSTEM = `You are the creative director and master perfumer of Kilau Bali, a boutique batch-atelier fragrance house.
...
1. Brand conception — a market-ready Kilau Bali name tailored to the scent's profile. Two words, Title Case. Draw on materials, light, minerals, weather, places and moods (house examples: Smoky Obsidian, Velvet Absolute, Golden Aura, Midnight Velvet, Fiery Spice, Azure Edge, Desert Nomad). Never reuse the reference brand or fragrance name, never use trademarked words, and avoid any name already in the house catalogue.
...
function normalise(raw: Record<string, unknown>): Conception {
  ...
  return {
    ...
    liquid: hex(raw.liquid, "#e3d8c9"),
    accent: hex(raw.accent, "#8a6215"),
