# Setting up Supabase for Kilau Bali
...
1. Supabase dashboard → **New project**. Pick an org, name (e.g. `kilau-bali`),
...
### Auth email: send it as Kilau Bali, via Mailgun
...
1. **Mailgun → Sending → Domains → Add New Domain.** Use a subdomain —
   `mg.kilaubali.com` — so the house's own mail records stay untouched.
...
2. **Add the DNS records Mailgun lists** at the registrar for
   `kilaubali.com`: the SPF `TXT`, the DKIM `TXT` on the selector
   ...
   **Strip the zone off each name before you paste it.** Mailgun prints names in
   ...
   `mg.kilaubali.com.kilaubali.com` and nothing verifies:

   | Record | Mailgun prints | Enter as Name |
   | --- | --- | --- |
   | SPF `TXT` | `mg.kilaubali.com` | `mg` |
   | DKIM `TXT` | `<selector>._domainkey.mg.kilaubali.com` | `<selector>._domainkey.mg` |
   | Tracking `CNAME` | `email.mg.kilaubali.com` | `email.mg` |
   | `MX` | `mg.kilaubali.com` | `mg` |
   ...
   does take full names wants `mg.kilaubali.com.` with the dot.
   ...
   ```bash
   dig TXT mg.kilaubali.com +short
   dig CNAME email.mg.kilaubali.com +short
   ```
...
3. **Supabase → Project Settings → Authentication → SMTP Settings**, enable
   ...
   | Port | `587` |
   | Username | the domain's SMTP login, e.g. `postmaster@mg.kilaubali.com` |
   | Password | that login's SMTP password (Mailgun → Domain settings → SMTP credentials — **not** the API key) |
   | Sender email | `no-reply@mg.kilaubali.com` |
   | Sender name | `Kilau Bali` |
...
supabase secrets set \
  ...
  AUSPOST_FROM_NAME="Kilau Bali" AUSPOST_FROM_LINE1="1 Atelier Ln" \
...
- **Stripe (hosted Checkout)** — migration `0015_stripe.sql` plus four Vercel variables
  ...
  1. In Vercel → Settings → Environment Variables add:
     ...
     (`https://kilau-bali.vercel.app`), and optionally `STRIPE_CURRENCY` (default `aud`).
