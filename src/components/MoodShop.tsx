...
export default function MoodShop({ fragrances, onQuickView }: MoodShopProps) {
  ...
  return (
    <section aria-label="Shop by mood" style={{ borderBottom: "1px solid #e4ddd0" }}>
      <Container style={{ padding: "26px 32px 26px" }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderRadius: 999,
                  border: `1px solid ${active ? GOLD : "rgba(20,18,14,0.16)"}`,
                  background: active ? "rgba(200,160,99,0.12)" : "none",
                  color: CREAM,
                  cursor: "pointer",
                  fontFamily: SERIF,
                  fontSize: 15,
              ...
              >
                <span aria-hidden style={{ width: 28, height: 28, borderRadius: "50%", background: `radial-gradient(circle at 35% 35%, ${m.swatch}ee, #ffffff)` }} />
                {id}
              </button>
            );
          })}
        ...
        <div className="kb-mood-grid" style={{ marginTop: 18, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
          {shown.map((f) => (
            <ScentCard key={f.id} frag={f} onQuickView={onQuickView} />
          ))}
          {shown.length === 0 && <p style={{ gridColumn: "1 / -1", fontSize: 13, color: "rgba(20,18,14,0.74)" }}>Nothing in this mood yet — try another.</p>}
        </div>
      </Container>
    </section>
  );
