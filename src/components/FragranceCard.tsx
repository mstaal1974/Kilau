  ...
  return (
    <article className="kb-card" style={{ border: "1px solid #e4ddd0", background: "#f7f4ee", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "16px 18px 18px", display: "flex", flexDirection: "column", gap: 9, flex: 1 }}>
        ...
        <div style={{ ...micro, color: "rgba(20,18,14,0.82)", fontSize: 8.5 }}>{profileOf(frag).join(" · ")}</div>
        <div style={{ fontSize: 12.5, color: "rgba(20,18,14,0.68)" }}>{notes}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, ...micro, fontSize: 8, whiteSpace: "nowrap", letterSpacing: "0.14em" }}>
          ...
          {availableIn(frag).map((a) => (
            <span key={a.group} style={{ color: a.status === "live" ? GOLD : "rgba(20,18,14,0.68)" }}>
        ...
        <div style={{ marginTop: "auto", paddingTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
          <button style={{ ...btnLink, color: locked ? "rgba(20,18,14,0.68)" : GOLD, whiteSpace: "nowrap", fontSize: 9 }} onClick={() => (locked ? navigate(paths.about) : onQuickView(frag, defaultFormat))}>
          ...
          {onToggleDiscovery && !locked && (
            <button
              ...
              style={{ background: "none", border: 0, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6, fontFamily: MONO, fontSize: 8, letterSpacing: "0.12em", textTransform: "uppercase", whiteSpace: "nowrap", color: inDiscovery ? GOLD : "rgba(20,18,14,0.74)" }}
            >
              <Icon name="heart" size={13} color={inDiscovery ? GOLD : "rgba(20,18,14,0.74)"} />
