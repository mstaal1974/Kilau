...
import { SD, cyanA, goldA, goldLeafA } from "./theme";

/**
 * The ambient field behind the whole experience: paper ground, three drifting
...
export default function Atmosphere({ intensity = 1 }: { intensity?: number }) {
  ...
  return (
    <div aria-hidden style={{ position: "fixed", inset: 0, zIndex: 0, overflow: "hidden", background: SD.paper, pointerEvents: "none" }}>
      <div
        style={{
          position: "absolute",
          inset: 0,
          background: `radial-gradient(78% 52% at 12% 0%, rgba(233,238,246,0.8) 0%, rgba(233,238,246,0) 64%),
                       radial-gradient(64% 46% at 92% 16%, rgba(31,111,139,0.08) 0%, rgba(31,111,139,0) 62%),
                       radial-gradient(90% 62% at 50% 108%, rgba(200,160,99,0.22) 0%, rgba(200,160,99,0) 64%),
                       linear-gradient(180deg, ${SD.paper} 0%, ${SD.mist} 48%, ${SD.paper} 100%)`,
        }}
      />
      <div className="sd-vapour sd-vapour-a" style={{ top: "-14%", left: "-10%", width: "62vw", height: "62vw", background: `radial-gradient(circle, ${cyanA(0.16 * intensity)} 0%, rgba(127,208,240,0) 68%)` }} />
      <div className="sd-vapour sd-vapour-b" style={{ top: "24%", right: "-16%", width: "54vw", height: "54vw", background: `radial-gradient(circle, ${goldLeafA(0.3 * intensity)} 0%, rgba(200,160,99,0) 68%)` }} />
      <div className="sd-vapour sd-vapour-c" style={{ bottom: "-22%", left: "22%", width: "70vw", height: "70vw", background: `radial-gradient(circle, rgba(31,111,139,${0.12 * intensity}) 0%, rgba(233,238,246,0) 70%)` }} />
      ...
      <svg viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.26 * intensity }}>
        <g className="sd-lattice">
          ...
          {molecules.nodes.map((n, i) => (
            <circle key={i} cx={n.x} cy={n.y} r={n.r} fill={i % 4 === 0 ? goldLeafA(0.75) : cyanA(0.45)} />
          ))}
        </g>
      </svg>

      {/* The horizon: a faceted ridge of paper with gold edge lighting. */}
      <svg viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ position: "absolute", left: 0, right: 0, bottom: 0, width: "100%", height: "34vh", minHeight: 180 }}>
        <defs>
          <linearGradient id="sdRock" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="100%" stopColor={SD.paper} />
          </linearGradient>
        </defs>
        ...
        <path d="M168 150 168 320M486 128 486 320M792 138 792 320M1104 146 1104 320" stroke="rgba(20,18,14,0.05)" strokeWidth="1" />
      </svg>
    </div>
  );
}
