...
const HOUSE_DOMAIN = "kilaubali.com";
...
export default function ShareCard({ print, url, code, topMatch }: ShareProps) {
  ...
  const share = useCallback(async () => {
    ...
    try {
      ...
      const file = blob ? new File([blob], "kilau-bali-scent-dna.png", { type: "image/png" }) : null;
      if (file && navigator.canShare?.({ files: [file] })) {
        await navigator.share({ files: [file], title: "My Scent DNA — Kilau Bali", text });
        return;
      }
      if (navigator.share) {
        await navigator.share({ title: "My Scent DNA — Kilau Bali", text, url });
        return;
      }
      await copy(url);
  ...
  const download = useCallback(async () => {
    ...
    a.href = href;
    a.download = `kilau-bali-scent-dna-${code}.png`;
    a.click();
    ...
    <div className="sd-share-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 320px) 1fr", gap: 34, alignItems: "start" }}>
      <div style={{ ...glass, padding: 12 }}>
        <canvas
          ...
          style={{ display: "block", width: "100%", height: "auto", background: SD.paper }}
        />
      </div>
      <div>
...
function drawCard(canvas: HTMLCanvasElement, print: Scentprint, topMatch: { name: string; percent: number } | null): void {
  ...
  ctx.fillStyle = SD.paper;
  ctx.fillRect(0, 0, W, H);
  const top = ctx.createRadialGradient(240, 140, 20, 240, 140, 900);
  top.addColorStop(0, "rgba(31,79,122,0.9)");
  top.addColorStop(1, "rgba(31,79,122,0)");
  ctx.fillStyle = top;
  ctx.fillRect(0, 0, W, H);
  const cyan = ctx.createRadialGradient(900, 340, 10, 900, 340, 760);
  cyan.addColorStop(0, "rgba(31,111,139,0.62)");
  cyan.addColorStop(1, "rgba(31,111,139,0)");
  ctx.fillStyle = cyan;
  ctx.fillRect(0, 0, W, H);
  const gold = ctx.createRadialGradient(540, 1380, 40, 540, 1380, 860);
  gold.addColorStop(0, "rgba(138,98,21,1)");
  gold.addColorStop(1, "rgba(138,98,21,1)");
  ctx.fillStyle = gold;
  ctx.fillRect(0, 0, W, H);

  // Frame
  ctx.strokeStyle = "rgba(138,98,21,1)";
  ctx.lineWidth = 1.5;
  ...
  tracked(ctx, "KILAU BALI", W / 2, 138, 9);
  ctx.fillStyle = "rgba(138,98,21,1)";
  ...
  ctx.strokeStyle = "rgba(20,18,14,0.08)";
  ctx.lineWidth = 1;
  ...
  fill.addColorStop(0, "rgba(31,111,139,0.68)");
  fill.addColorStop(0.6, "rgba(31,111,139,0.14)");
  fill.addColorStop(1, "rgba(138,98,21,1)");
  ctx.fillStyle = fill;
  ctx.fill();
  ctx.save();
  ctx.shadowColor = "rgba(138,98,21,1)";
  ctx.shadowBlur = 26;
  ...
  points.forEach((p) => {
    ...
    ctx.fillStyle = p.value >= 60 ? "rgba(122,90,18,0.95)" : "rgba(20,18,14,0.68)";
  ...
  ctx.fillStyle = "rgba(20,18,14,0.74)";
  ...
  four.forEach((d, i) => {
    ...
    ctx.fillStyle = "rgba(20,18,14,0.68)";
  ...
  if (topMatch) {
    ctx.fillStyle = "rgba(31,111,139,0.9)";
    ctx.font = "12px 'Space Mono', monospace";
