...
const INTRO: Record<CollectionMode, { eyebrow: string; title: string; copy: string; art?: string; fallback?: string }> = {
  ...
  car: { eyebrow: "Kilau Drive", title: "Your fragrance. Your car.", copy: "Every scent in the house, in our handcrafted wooden-cap car diffuser. Same iconic scents — a bolder journey.", art: "/assets/banner-drive.jpg", fallback: "/assets/bottle-portrait.webp" },
  body: { eyebrow: "Kilau Ritual", title: "Cleanse. Hydrate. Be obsessed.", copy: "Body wash, moisturiser and the Complete Ritual set. Layer the fragrance from morning to night.", art: "/assets/banner-ritual.jpg", fallback: "/assets/bottle-pair.png" },
};
...
export default function Collection({ mode, facet, fragrances, vip, discoveryIds, onQuickView, onToggleDiscovery }: CollectionProps) {
  ...
  return (
    <main data-screen-label={intro.title}>
      {intro.art ? (
        <Art src={intro.art} fallback={intro.fallback} alt="" position="right center" style={{ minHeight: 240, borderBottom: "1px solid #e4ddd0" }}>
      ...
      <Container style={{ padding: "18px 32px 60px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", borderBottom: "1px solid #e4ddd0", paddingBottom: 14 }}>
          ...
          <input
            ...
            style={{ background: "none", border: "1px solid #b8873c", outline: "none", color: CREAM, fontFamily: MONO, fontSize: 10.5, letterSpacing: "0.02em", padding: "7px 10px", width: 190 }}
          />
          <span style={{ marginLeft: "auto", fontFamily: MONO, fontSize: 10, color: "rgba(20,18,14,0.68)" }}>{list.length} of {fragrances.length}</span>
        </div>
