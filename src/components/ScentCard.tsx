...
export default function ScentCard({ frag, onQuickView }: ScentCardProps) {
  ...
  return (
    <article className="kb-card" style={{ border: "1px solid #e4ddd0", background: "#f7f4ee", display: "grid", gridTemplateColumns: "150px 1fr", minHeight: 176 }}>
      <button onClick={() => navigate(paths.product(frag.slug))} aria-label={`Open ${frag.name}`} style={{ padding: 0, border: 0, background: "none", cursor: "pointer", borderRight: "1px solid #e4ddd0" }}>
      ...
      <div style={{ padding: "16px 18px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(20,18,14,0.68)", lineHeight: 1.45, textTransform: "lowercase" }}>{notes}</div>
