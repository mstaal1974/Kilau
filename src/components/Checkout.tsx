...
export default function Checkout({ lines, fragrances, email, signedIn, onSignIn, busy, error, cancelled, onPlaceOrder }: CheckoutProps) {
  ...
  const shippingCell = alternate
    ? { value: "Free", note: "Alternate delivery — arranged with you", tone: "#cfe4ce" }
    : postageOff
      ...
      : ship?.status === "loading"
        ...
        : ship?.status === "error"
          ? { value: "Unavailable", note: ship.error ?? "", tone: "#f0d3c5" }
          : rate
            ? { value: rate.chargeCents === 0 ? "Free" : moneyExact(rate.chargeCents), note: rate.chargeCents === 0 ? `Australia Post · ${rate.name}` : `Australia Post · ${rate.name} · incl. GST ${moneyExact(rate.gstCents)}`, tone: rate.chargeCents === 0 ? "#cfe4ce" : CREAM }
            : { value: "Enter postcode", note: "", tone: "rgba(20,18,14,0.68)" };
  ...
  if (!rows.length) {
    return (
      <main data-screen-label="Checkout" style={{ maxWidth: 1340, margin: "0 auto", padding: "120px 32px", textAlign: "center" }}>
        ...
        <p style={{ marginTop: 12, fontSize: 14, color: "rgba(20,18,14,0.68)" }}>One scent, every part of your day — start with the one you'd wear.</p>
        <button className="kb-cta" style={{ ...btnGold, marginTop: 26 }} onClick={() => navigate(paths.fragrances)}>
  ...
  return (
    <main data-screen-label="Checkout" style={{ maxWidth: 1160, margin: "0 auto", padding: "116px 32px 90px" }}>
      ...
      {cancelled && (
        <p style={{ marginTop: 14, border: "1px solid rgba(184,135,60,0.38)", padding: "12px 16px", fontSize: 14, lineHeight: 1.6, color: "rgba(20,18,14,0.82)" }}>
          You came back without paying — nothing was charged. Your bag is exactly as you left it.
        </p>
      )}

      <div className="kb-checkout-grid" style={{ marginTop: 30, display: "grid", gridTemplateColumns: "minmax(0,1fr) 400px", gap: 34, alignItems: "start" }}>
        {/* ── Details ── */}
        <div style={{ display: "grid", gap: 26 }}>
          <div>
            ...
            <p style={{ margin: "8px 0 0", fontSize: 14, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>
              {signedIn ? (
          ...
          <div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {(
                ...
                <label key={o.key} style={{ display: "flex", alignItems: "flex-start", gap: 11, cursor: "pointer", border: `1px solid ${method === o.key ? "rgba(184,135,60,0.45)" : "#e4ddd0"}`, padding: "13px 14px" }}>
                  <input type="radio" name="kb-delivery" checked={method === o.key} onChange={() => setMethod(o.key)} style={{ marginTop: 3, accentColor: "#8a6215" }} />
                  <span>
                    <span style={{ display: "block", fontFamily: SERIF, fontSize: 18, color: CREAM, lineHeight: 1.2 }}>{o.title}</span>
                    <span style={{ display: "block", marginTop: 3, fontSize: 14, lineHeight: 1.5, color: "rgba(20,18,14,0.68)" }}>{o.body}</span>
                  </span>
                </label>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              ...
              {alternate ? (
                ...
                <>
                  ...
                  <div className="kb-checkout-triple" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 0.9fr", gap: 10 }}>
          ...
          {!alternate && rates && rates.length > 1 && (
            <div>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                {rates.map((r) => (
                  <label key={r.code} style={{ display: "flex", alignItems: "center", gap: 11, cursor: "pointer", border: `1px solid ${rate?.code === r.code ? "rgba(184,135,60,0.45)" : "#e4ddd0"}`, padding: "11px 14px" }}>
                    <input type="radio" name="kb-postage" checked={rate?.code === r.code} onChange={() => setChosen(r.code)} style={{ accentColor: "#8a6215" }} />
                    ...
                    <span style={{ fontFamily: MONO, fontSize: 14, color: r.chargeCents === 0 ? "#2f6b3e" : CREAM }}>{r.chargeCents === 0 ? "Free" : moneyExact(r.chargeCents)}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start", border: "1px solid #e4ddd0", padding: "14px 16px" }}>
              <Icon name="lock" size={15} color={GOLD} />
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.65, color: "rgba(20,18,14,0.74)" }}>
        ...
        <aside style={{ border: "1px solid #e4ddd0", background: "#f7f4ee", padding: 22, display: "grid", gap: 14, position: "sticky", top: 100 }}>
          ...
          <div style={{ display: "grid", gap: 6, borderTop: "1px solid #e4ddd0", paddingTop: 14 }}>
            ...
            {shippingCell.note && <div style={{ fontSize: 14, lineHeight: 1.5, color: shippingCell.tone === "#a8431b" ? "#a8431b" : "rgba(20,18,14,0.68)" }}>{shippingCell.note}</div>}
          </div>

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", borderTop: "1px solid #e4ddd0", paddingTop: 14, fontFamily: MONO, color: CREAM }}>
            <span style={micro}>Total</span>
            <span style={{ fontFamily: SERIF, fontSize: 30 }}>{moneyExact(total)}</span>
          </div>

          <button className="kb-cta" style={{ ...btnGold, minHeight: 48, height: "auto", padding: "14px 18px", fontSize: 14, letterSpacing: "0.06em", flexWrap: "wrap", justifyContent: "center", opacity: busy || (!alternate && (!rate || postageOff)) ? 0.6 : 1 }} disabled={busy || (!alternate && (!rate || postageOff))} onClick={place}>
          ...
          <p style={{ margin: 0, fontSize: 14, lineHeight: 1.55, color: "rgba(20,18,14,0.68)", textAlign: "center" }}>Secured by Stripe · you'll confirm payment on the next step.</p>
          {(showError && formError) || error ? <div style={{ fontSize: 14, lineHeight: 1.55, color: "#a8431b" }}>{error ?? formError}</div> : null}
          <div style={{ ...micro, fontSize: 14, display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            <span>
              <Icon name="truck" size={12} color="rgba(20,18,14,0.74)" /> Free shipping over $100
            </span>
            <span>
              <Icon name="refresh" size={12} color="rgba(20,18,14,0.74)" /> 30-day returns
            </span>
          </div>
        </aside>
      </div>
