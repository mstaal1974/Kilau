...
export default function Discovery({ fragrances, vip, discoveryIds, onToggleDiscovery, onAddBox, onQuickView }: DiscoveryProps) {
  ...
  return (
    <main data-screen-label="Discovery">
      <div style={{ borderBottom: "1px solid #e4ddd0", background: "linear-gradient(180deg, #fcfaf6, #ffffff)" }}>
        <Container style={{ padding: "48px 32px 36px", display: "grid", gridTemplateColumns: "1fr 420px", gap: 40, alignItems: "start" }} >
          ...
          {/* Build your 5 */}
          <aside aria-label="Your Discovery Box" style={{ border: `1px solid ${full ? GOLD : "rgba(184,135,60,0.3)"}`, background: "#f7f4ee", padding: 22 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, marginTop: 16 }}>
              {Array.from({ length: DISCOVERY_BOX_SIZE }).map((_, i) => {
                const f = picked[i];
                return f ? (
                  <button key={f.id} onClick={() => onToggleDiscovery(f)} title={`Remove ${f.name}`} style={{ padding: 0, border: "1px solid rgba(184,135,60,0.45)", background: "none", cursor: "pointer" }}>
                  ...
                  <span key={i} aria-hidden style={{ height: 76, border: "1px dashed #d8d0c0", display: "grid", placeItems: "center", color: "rgba(20,18,14,0.62)", fontFamily: MONO, fontSize: 10 }}>{i + 1}</span>
                );
              })}
            </div>
            <ul style={{ margin: "12px 0 0", padding: 0, listStyle: "none", display: "grid", gap: 4 }}>
              {picked.map((f) => (
                <li key={f.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, color: "rgba(20,18,14,0.82)" }}>
                  <span>{f.name}</span>
                  <span style={{ ...micro, fontSize: 8 }}>{profileOf(f).slice(0, 2).join(" · ")}</span>
                </li>
              ))}
            </ul>
            <button className="kb-cta" style={{ ...btnGold, width: "100%", justifyContent: "center", marginTop: 16, opacity: full ? 1 : 0.5 }} disabled={!full} onClick={() => onAddBox(picked)}>
