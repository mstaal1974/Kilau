 ...
 * paper scrim so the layout holds even without imagery.
 *
 * The scrim is what makes near-black type safe over photography we do not
 * control: it is heaviest where the copy sits (0.88–0.96 white keeps ink above
 * 12:1 whatever the shot underneath) and thins out over the product itself.
 */
export function Art({
  src,
  ...
  position = "center",
  style,
  overlay = "linear-gradient(90deg, rgba(255,255,255,0.96) 0%, rgba(255,255,255,0.9) 45%, rgba(255,255,255,0.74) 100%)",
  children,
}: {
  src: string;
  fallback?: string;
  ...
  return (
    <div style={{ position: "relative", overflow: "hidden", background: "radial-gradient(70% 60% at 60% 50%, #e6dccd 0%, #faf8f3 70%)", ...style }}>
      <img
        src={cur}
        alt={alt}
        loading="lazy"
        onError={() => cur !== fallback && setCur(fallback)}
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: position, opacity: cur === fallback ? 0.45 : 1, filter: cur === fallback ? "brightness(1.08)" : undefined }}
      />
...
export function SideCaption({ lines, style }: { lines: string[]; style?: CSSProperties }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4, ...style }}>
      {lines.map((l, i) =>
        l === "—" ? (
          <span key={i} style={{ display: "block", width: 34, height: 1, background: "rgba(200,160,99,0.6)", margin: "16px 0" }} />
        ) : (
          <span key={i} style={{ ...micro, color: i < lines.indexOf("—") || !lines.includes("—") ? "#7a5a12" : "rgba(20,18,14,0.74)", letterSpacing: "0.34em" }}>
            {l}
          </span>
        ),
      )}
...
export function IconBadge({ name, label }: { name: IconName; label: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 9 }}>
      <span style={{ width: 44, height: 44, border: "1px solid rgba(184,135,60,0.45)", borderRadius: "50%", display: "grid", placeItems: "center" }}>
        <Icon name={name} />
      </span>
  ...
  return (
    <Tag
      ...
      style={{
        ...
        background: active ? "rgba(200,160,99,0.14)" : "none",
        border: `1px solid ${active || tone === "gold" ? "rgba(184,135,60,0.52)" : "rgba(20,18,14,0.16)"}`,
        color: active || tone === "gold" ? GOLD : "rgba(20,18,14,0.9)",
  ...
  return (
    <div
      style={{
        ...
        width: "100%",
        boxSizing: "border-box",
        border: "1px solid rgba(184,135,60,0.56)",
        borderLeft: `4px solid ${GOLD}`,
        background: "linear-gradient(90deg, rgba(200,160,99,0.22), rgba(200,160,99,0.06))",
        padding: size === "lg" ? "10px 16px 11px 14px" : "8px 12px 9px 10px",
        ...style,
      }}
    >
      <span style={{ ...micro, color: "#7a5a12", fontSize: size === "lg" ? 9.5 : 8.5, letterSpacing: "0.32em", whiteSpace: "nowrap" }}>Inspired by</span>
      <span style={{ fontFamily: SERIF, fontSize: px, lineHeight: 1.1, color: "#7a5a12", fontWeight: 600, letterSpacing: "0.01em" }}>
        {brand}
        {fragrance ? <span style={{ opacity: 0.85, fontWeight: 400 }}> — </span> : null}
        {fragrance ? <span style={{ fontStyle: "italic", fontWeight: 500, color: "#6b4e10" }}>{fragrance}</span> : null}
      </span>
    </div>
  );
}
