...
export default function ProductDetail({ frag, fragrances, vip, onAdd, onQuickView }: ProductDetailProps) {
  ...
  const option = (s: Sku) => {
    ...
    return (
      ...
      >
        ...
        <span style={{ fontFamily: MONO, fontSize: 10.5, color: soon ? GOLD : "rgba(20,18,14,0.9)", letterSpacing: "0.02em" }}>
          {soon ? (notified.has(s.key) ? "Notified ✓" : "Notify me") : (
            <>
              {money(s.price)}
              {s.compareAt && <s style={{ marginLeft: 8, color: "rgba(20,18,14,0.68)" }}>{money(s.compareAt)}</s>}
            </>
          )}
        </span>
      </button>
  ...
  return (
    <main data-screen-label="Product">
      ...
      <div className="kb-pdp-grid" style={{ display: "grid", gridTemplateColumns: "0.72fr 1.28fr", borderBottom: "1px solid #e4ddd0" }}>
        {/* HERO IMAGE */}
        <div style={{ borderRight: "1px solid #e4ddd0", padding: "18px 24px 22px 32px" }}>
          <div style={{ position: "relative", background: bottleBackdrop(frag.accent, frag.liquid), border: "1px solid #e4ddd0" }}>
            ...
            <SideCaption lines={[...profile, "—", "A bolder", "you"]} style={{ position: "absolute", left: 18, top: 20, background: "rgba(255,255,255,0.55)", padding: "10px 12px", backdropFilter: "blur(2px)" }} />
          </div>
        </div>
        ...
        <div style={{ padding: "18px 32px 26px 30px", display: "grid", gridTemplateColumns: "1fr auto", gap: 18 }}>
          <div style={{ minWidth: 0 }}>
            <nav aria-label="Breadcrumb" style={{ ...micro, fontSize: 8.5, display: "flex", gap: 6 }}>
              ...
              <button style={{ ...btnLink, color: "rgba(20,18,14,0.68)", fontSize: 8.5 }} onClick={() => navigate(paths.fragrances)}>Fragrances</button> /
              <span style={{ color: "rgba(20,18,14,0.82)" }}>{frag.name}</span>
            </nav>
            ...
            <p style={{ margin: "10px 0 0", fontFamily: SERIF, fontSize: 17.5, lineHeight: 1.45, color: "rgba(20,18,14,0.82)", maxWidth: 620 }}>{frag.story}</p>
            ...
            <div className="kb-formats-grid" style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "stretch" }}>
              {ORDER.map((g) => {
                const list = skusInGroup(frag, g);
                if (!list.length) return null;
                return (
                  <div key={g} style={{ border: "1px solid #d8d0c0", padding: "12px 10px 10px", background: "#fcfaf6", flex: "0 0 auto" }}>
                    ...
                    <div style={{ ...micro, fontSize: 8, marginTop: 3, color: "rgba(20,18,14,0.74)" }}>{GROUPS[g].sub}</div>
            ...
            {/* Summary + add */}
            <div style={{ marginTop: 18, borderTop: "1px solid #e4ddd0", paddingTop: 16, display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 220 }}>
                <span style={{ width: 62, height: 78, flexShrink: 0, display: "grid", placeItems: "center", background: bottleBackdrop(frag.accent, frag.liquid), border: "1px solid #e4ddd0" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12.5, color: "rgba(20,18,14,0.74)", marginTop: 4, letterSpacing: "0.04em" }}>{chosen.def.name}</div>
              ...
              >
                <Icon name="bag" size={14} color="#14120e" /> {locked ? "VIP members only" : chosen.buyable ? "Add to bag" : chosen.status === "coming_soon" ? "Coming soon" : "Sold out"}
              </button>
              <button aria-label="Save to wishlist" style={{ width: 52, height: 52, border: "1px solid rgba(184,135,60,0.38)", background: "none", cursor: "pointer", display: "grid", placeItems: "center" }}>
                <Icon name="heart" size={16} />
              </button>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginTop: 10, flexWrap: "wrap", ...micro, fontSize: 8.5 }}>
              <span style={{ color: chosen.status === "live" && (chosen.stock > 0 || chosen.def.group === "wear") ? "#2f6b3e" : GOLD }}>
                ● {chosen.availability}
          ...
          <SideCaption lines={["Iconic", "scents", "—", "A bolder", "you"]} style={{ paddingTop: 30, borderLeft: "1px solid #e4ddd0", paddingLeft: 14, width: 62 }} />
        </div>
      </div>

      {/* ── Fragrance notes ── */}
      <section aria-label="Fragrance notes" style={{ borderBottom: "1px solid #e4ddd0", background: "#fcfaf6" }}>
        <Container style={{ padding: "22px 32px" }}>
          <div className="kb-notes-grid" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 1fr 1fr auto", gap: 22, alignItems: "center" }}>
            <div style={{ borderRight: "1px solid #e4ddd0", paddingRight: 22 }}>
              <h2 style={{ margin: 0, fontFamily: SERIF, fontWeight: 400, fontSize: 32, color: CREAM }}>Fragrance notes</h2>
              <p style={{ margin: "8px 0 0", fontSize: 13, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>{frag.tagline} A composition of rare elements, balanced to perfection.</p>
            </div>
            {([
              ["Top notes", frag.top, `linear-gradient(135deg, ${frag.accent}66, #efeae0)`],
              ["Heart notes", frag.heart, `linear-gradient(135deg, ${frag.liquid}, #2c1a0c)`],
              ["Base notes", frag.base, "linear-gradient(135deg, #e3d8c9, #faf8f3)"],
            ] as const).map(([title, notes, bg]) => (
              <div key={title} style={{ display: "grid", gridTemplateColumns: "88px 1fr", gap: 16, alignItems: "center" }}>
                <span aria-hidden style={{ height: 88, background: bg, border: "1px solid #e4ddd0" }} />
                <div>
                  <div style={{ ...micro, color: CREAM, fontSize: 9 }}>{title}</div>
                  <ul style={{ margin: "8px 0 0", padding: 0, listStyle: "none", fontSize: 13.5, lineHeight: 1.5, color: "rgba(20,18,14,0.9)" }}>
                    {notes.map((n) => <li key={n}>{n}</li>)}
                  </ul>
                </div>
              </div>
            ))}
            <SideCaption lines={["Depth", "in every", "layer"]} style={{ borderLeft: "1px solid #e4ddd0", paddingLeft: 22 }} />
          </div>
        </Container>
      </section>
      ...
      {alsoAvailable.length > 0 && (
        <section aria-label={`Also available in ${frag.name}`} style={{ borderBottom: "1px solid #e4ddd0" }}>
          <Container style={{ padding: "22px 32px 24px" }}>
            ...
            <div className="kb-also-grid" style={{ marginTop: 14, display: "grid", gridTemplateColumns: `repeat(${alsoAvailable.length}, 1fr)`, gap: 12 }}>
              {alsoAvailable.map((s) => (
                <button
                  key={s.key}
                  onClick={() => { setKey(s.key); window.scrollTo({ top: 0, behavior: "smooth" }); }}
                  style={{ display: "grid", gridTemplateColumns: "110px 1fr", gap: 14, alignItems: "center", border: "1px solid #e4ddd0", background: "#f7f4ee", padding: 0, textAlign: "left", cursor: "pointer", color: CREAM, minHeight: 116 }}
                >
                  <span style={{ height: "100%", background: bottleBackdrop(frag.accent, frag.liquid), display: "grid", placeItems: "center", borderRight: "1px solid #e4ddd0", padding: 8 }}>
                  ...
                  <span style={{ padding: "12px 14px 12px 0" }}>
                    ...
                    <span style={{ display: "block", fontSize: 12, color: "rgba(20,18,14,0.74)", marginTop: 3 }}>
                    ...
                    <span style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                      ...
                      <span style={{ width: 26, height: 26, borderRadius: "50%", border: "1px solid rgba(184,135,60,0.52)", display: "grid", placeItems: "center", color: GOLD }}><Arrow size={10} /></span>
                    </span>
                  </span>
                </button>
              ))}
      ...
      <section aria-label="You may also like">
        <Container style={{ padding: "22px 32px 60px" }}>
          ...
          <div className="kb-vault-grid" style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
            {related.map((r) => (
