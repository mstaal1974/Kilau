...
export default function AdminMarketing({ fragrances, configured }: { fragrances: Fragrance[]; configured: boolean }) {
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `kilau-bali-audience${mood ? `-${mood.toLowerCase()}` : ""}.csv`;
    a.click();
  ...
  return (
    <section>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h2 style={{ margin: 0, fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 30, color: "#14120e" }}>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "rgba(20,18,14,0.68)", maxWidth: 820, lineHeight: 1.6 }}>
      ...
      <div style={{ marginTop: 16, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button onClick={() => setMood(null)} style={{ ...chip, cursor: "pointer", borderColor: !mood ? GOLD : "#e4ddd0", color: !mood ? GOLD : "rgba(20,18,14,0.74)" }}>all</button>
        {MOODS.map((m) => {
          const n = optedIn.filter((x) => x.profile.moods.some((y) => y.id === m.id)).length;
          return (
            <button key={m.id} onClick={() => setMood(mood === m.id ? null : m.id)} style={{ ...chip, cursor: "pointer", borderColor: mood === m.id ? GOLD : "#e4ddd0", color: mood === m.id ? GOLD : "rgba(20,18,14,0.74)" }}>
      ...
      <div className="kb-marketing-grid" style={{ marginTop: 22, display: "grid", gridTemplateColumns: "1fr 380px", gap: 20, alignItems: "start" }}>
        <div style={{ border: "1px solid #e4ddd0" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1.6fr 110px 130px", gap: 14, padding: "10px 16px", borderBottom: "1px solid #e4ddd0" }}>
            {["Customer", "Taste", "Consent", "Opted in"].map((h) => (
              <span key={h} style={label}>{h}</span>
            ))}
          </div>
          {loading && !shown.length ? (
            <div style={{ padding: 22, fontSize: 12, color: "rgba(20,18,14,0.68)" }}>Loading…</div>
          ) : !shown.length ? (
            <div style={{ padding: 22, fontSize: 12, color: "rgba(20,18,14,0.68)", lineHeight: 1.6 }}>
        ...
        <aside style={{ border: "1px solid rgba(184,135,60,0.34)", background: "#f7f4ee", padding: 20, display: "grid", gap: 12, position: "sticky", top: 90 }}>
          <div style={label}>Draft a note to this segment</div>
          <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>{segmentText}</p>
          ...
          <button className="kb-cta" style={{ ...btnGold, height: 40 }} disabled={drafting || !shown.length} onClick={() => void write()}>
            {drafting ? "Writing…" : "Draft with AI"}
          </button>
          {draft && (
            <div style={{ borderTop: "1px solid #e4ddd0", paddingTop: 12, display: "grid", gap: 8 }}>
              <div style={{ ...label, color: draft.source === "claude" ? GOLD : "rgba(20,18,14,0.68)" }}>{draft.source === "claude" ? "Drafted by Claude" : "House template (AI not configured)"}</div>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: "#14120e" }}>{draft.draft.subject}</div>
              <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10.5, color: "rgba(20,18,14,0.68)" }}>{draft.draft.preview}</div>
              <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.7, color: "rgba(20,18,14,0.82)", whiteSpace: "pre-wrap" }}>{draft.draft.body}</p>
...
function Row({ m }: { m: AudienceMember }) {
  const p = m.profile;
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1.3fr 1.6fr 110px 130px", gap: 14, padding: "12px 16px", borderBottom: "1px solid #e4ddd0", alignItems: "start" }}>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#14120e", wordBreak: "break-all" }}>{m.email}</div>
        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: "rgba(20,18,14,0.68)", marginTop: 4 }}>
          {m.purchases} purchase{m.purchases === 1 ? "" : "s"}
          {m.subscription ? ` · Monthly Pour ${m.subscription.status}` : ""}
          {m.source ? ` · via ${m.source}` : ""}
        </div>
      </div>
      <div style={{ fontSize: 12, lineHeight: 1.55, color: "rgba(20,18,14,0.74)" }}>
        {p.empty ? (
          <span style={{ color: "rgba(20,18,14,0.68)" }}>No history yet</span>
        ) : (
          <>
      ...
      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, lineHeight: 1.7 }}>
        <div style={{ color: "#2f6b3e" }}>email ✓</div>
        <div style={{ color: m.ai ? "#2f6b3e" : "rgba(20,18,14,0.62)" }}>{m.ai ? "AI ✓" : "AI —"}</div>
      </div>
      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10.5, color: "rgba(20,18,14,0.74)" }}>{fmtDate(m.optedInAt)}</div>
    </div>
  );
}
