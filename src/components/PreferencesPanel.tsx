...
export default function PreferencesPanel({ consents, taste, onChange }: PreferencesPanelProps) {
  ...
  return (
    <section style={{ marginTop: 44 }}>
      ...
      <p style={{ margin: "8px 0 0", fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)", maxWidth: 620 }}>
      ...
      <div style={{ marginTop: 20, border: "1px solid #e4ddd0", background: "#f7f4ee" }}>
        {rows.map((r, i) => (
          <label key={r.key} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "center", padding: "18px 22px", borderTop: i ? "1px solid #e4ddd0" : 0, cursor: "pointer" }}>
            <span>
              <span style={{ display: "block", fontFamily: SERIF, fontSize: 21, color: CREAM, lineHeight: 1.1 }}>{r.title}</span>
              <span style={{ display: "block", marginTop: 6, fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>{r.body}</span>
              {consents[r.key] && r.since && <span style={{ display: "block", marginTop: 6, fontFamily: MONO, fontSize: 10, color: "rgba(20,18,14,0.68)" }}>Opted in {fmtDate(r.since)}</span>}
            </span>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 10, fontFamily: MONO, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: consents[r.key] ? GOLD : "rgba(20,18,14,0.68)" }}>
      ...
      {consents.ai && (
        <div style={{ marginTop: 14, border: "1px dashed rgba(184,135,60,0.3)", padding: "16px 22px" }}>
          <div style={{ ...micro, color: GOLD }}>What the house has learned</div>
          {!taste ? (
            <p style={{ margin: "8px 0 0", fontSize: 12.5, color: "rgba(20,18,14,0.68)" }}>Reading your history…</p>
          ) : taste.empty ? (
            <p style={{ margin: "8px 0 0", fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>Nothing yet. As you order, subscribe or ask the concierge, your taste profile builds here, and you can see it.</p>
          ) : (
            <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
              {taste.moods.length > 0 && (
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {taste.moods.map((m) => (
                    <span key={m.id} style={{ fontFamily: MONO, fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: CREAM, border: "1px solid #e4ddd0", padding: "4px 8px" }}>{m.id}</span>
                  ))}
                </div>
              )}
              <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.7, color: "rgba(20,18,14,0.74)" }}>{taste.summary}</p>
            </div>
          )}
        </div>
      )}
