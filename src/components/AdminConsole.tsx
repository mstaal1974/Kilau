...
export default function AdminConsole({ fragrances, configured, onReload, demoCommits }: AdminConsoleProps) {
  const [tab, setTab] = useState<"setup" | "catalogue" | "matrix" | "fulfillment" | "dropship" | "requests" | "subscriptions" | "marketing">("catalogue");

  return (
    <main data-screen-label="Admin" style={{ maxWidth: 1340, margin: "0 auto", padding: "48px 32px 90px" }}>
      <div style={{ ...label, color: "rgba(138,98,21,1)", letterSpacing: "0.28em" }}>Atelier Admin</div>
      <h1 style={{ margin: "14px 0 0", fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 54, color: "#14120e" }}>
        The <span style={{ fontStyle: "italic", color: "#8a6215" }}>Console.</span>
      </h1>
      {!configured && (
        <p style={{ margin: "12px 0 0", fontSize: 11.5, color: "rgba(20,18,14,0.68)" }}>
      ...
      <div style={{ display: "flex", gap: 22, margin: "28px 0 30px", borderBottom: "1px solid #e4ddd0" }}>
        {(["setup", "catalogue", "matrix", "fulfillment", "dropship", "subscriptions", "requests", "marketing"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
              background: "none",
              border: 0,
              borderBottom: tab === t ? "1px solid #c8a063" : "1px solid transparent",
              cursor: "pointer",
              padding: "0 0 12px",
              color: tab === t ? "#8a6215" : "rgba(20,18,14,0.68)",
              fontSize: 11,
          ...
          >
            {t === "setup" ? "Setup" : t === "catalogue" ? "Catalogue & Inventory" : t === "matrix" ? "Product Matrix" : t === "fulfillment" ? "Fulfillment" : t === "dropship" ? "Dropship" : t === "subscriptions" ? "Monthly Pour" : t === "requests" ? "Requests" : "Marketing"}
          </button>
        ))}
      </div>
      ...
      {tab === "fulfillment" && (
        <p style={{ margin: "-10px 0 22px", fontSize: 11.5 }}>
          <a href="#/staff" style={{ color: "#8a6215", letterSpacing: "0.18em", textTransform: "uppercase", fontSize: 10, textDecoration: "none" }}>
            Open the staff order desk →
          </a>
          <span style={{ color: "rgba(20,18,14,0.68)", marginLeft: 10 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "rgba(20,18,14,0.68)" }}>
          {fragrances.length} fragrances
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button
            style={{ ...btnGhost, borderColor: "rgba(184,135,60,0.34)", color: "#8a6215" }}
            onClick={() => {
              setConceiving(true);
              setEditing(null);
            }}
      ...
      {saveError && (
        <p role="alert" style={{ margin: "0 0 14px", fontSize: 12.5, color: "#a8431b" }}>
          {saveError}
        </p>
      )}
  ...
  const stockBox = (v: number, set: (n: number) => void, size: string) => (
    <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <span style={{ ...label, fontSize: 8 }}>
        {size}
        {v <= low && <span style={{ color: "#a8431b" }}> · low</span>}
      </span>
      <input
        type="number"
        min={0}
        ...
        style={{ ...field, width: 62, height: 34, color: v <= low ? "#a8431b" : "#14120e" }}
      />
    </label>
  );
    ...
    >
      <div style={{ minWidth: 200, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, color: "#14120e", lineHeight: 1 }}>{f.name}</div>
      ...
      <div style={{ minWidth: 190 }}>
        ...
        <div style={{ marginTop: 5, display: "flex", gap: 12, fontFamily: "'Space Mono',monospace", fontSize: 12, color: "#14120e" }}>
          <span>10 · {counts[10]}</span>
          <span>30 · {counts[30]}</span>
          <span>50 · {counts[50]}</span>
        </div>
        <div style={{ marginTop: 6, fontFamily: "'Space Mono',monospace", fontSize: 11, color: covered ? "#2f6b3e" : "#a8431b" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button onClick={onDelete} style={{ ...btnGhost, height: 34, color: "#a8431b" }}>
          Delete
        </button>
      </div>
    </div>
...
function Editor({
  ...
  return (
    <div style={{ border: "1px solid rgba(184,135,60,0.26)", background: "rgba(244,240,232,0.5)", padding: 24 }}>
      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, color: "#14120e", marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1, minWidth: 220 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {(image || f.imageUrl) && (
                <button
                  type="button"
                  style={{ ...btnGhost, height: 34, color: "#a8431b" }}
                  onClick={() => {
                    setImage(null);
            ...
            <span style={{ fontSize: 11.5, lineHeight: 1.5, color: imageError ? "#a8431b" : "rgba(20,18,14,0.68)" }}>
              {imageError ??
                (image
...
function Fulfillment({
  ...
  if (configured && remote === null) {
    return <p style={{ fontSize: 13, color: "rgba(20,18,14,0.68)" }}>Loading commits…</p>;
  }
  if (commits.length === 0) {
    return <p style={{ fontSize: 13, color: "rgba(20,18,14,0.68)" }}>No commits to fulfill yet.</p>;
  }
...
function FulfillRow({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap", border: "1px solid #e4ddd0", background: "#f7f4ee", padding: "14px 18px" }}>
      <div>
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: "#14120e" }}>{name}</div>
        <div style={{ ...label, marginTop: 4 }}>
        ...
        {commit.delivery_method !== "alternate" && (commit.ship_address || commit.delivery_name) && (
          <div style={{ marginTop: 8, border: "1px solid #e4ddd0", padding: "8px 10px", maxWidth: 460 }}>
            <div style={{ ...label, color: GOLD }}>Ship to</div>
            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#14120e", marginTop: 4, lineHeight: 1.6 }}>
        ...
        {commit.delivery_method === "alternate" && (
          <div style={{ marginTop: 8, border: "1px solid rgba(184,135,60,0.34)", padding: "8px 10px", maxWidth: 460 }}>
            <div style={{ ...label, color: GOLD }}>Alternate delivery — arrange directly</div>
            <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#14120e", marginTop: 4 }}>
              {commit.delivery_name ?? "—"}
              {commit.delivery_phone ? ` · ${commit.delivery_phone}` : ""}
            </div>
            {commit.delivery_notes && (
              <div style={{ fontSize: 12, lineHeight: 1.5, color: "rgba(20,18,14,0.74)", marginTop: 4, whiteSpace: "pre-wrap" }}>{commit.delivery_notes}</div>
            )}
          </div>
        )}
      </div>
      {done ? (
        <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#2f6b3e" }}>✓ {done}</span>
      ) : (
