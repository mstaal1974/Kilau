...
export default function Subscribe({ fragrances, vip, initialSlug, initialFormat, hasActive, busy, started, error, onStart }: SubscribeProps) {
    ...
    return (
      <main data-screen-label="Subscribed" style={{ minHeight: "60vh" }}>
        <Container style={{ padding: "80px 32px 100px", maxWidth: 760 }}>
          ...
          <p style={{ margin: "18px 0 0", fontSize: 14.5, lineHeight: 1.7, color: "rgba(20,18,14,0.74)" }}>
          ...
          <div style={{ display: "flex", gap: 14, marginTop: 30, flexWrap: "wrap" }}>
            <button className="kb-cta" style={btnGold} onClick={() => navigate(paths.account)}>Manage my subscription <Arrow /></button>
  ...
  return (
    <main data-screen-label="Subscribe" style={{ minHeight: "60vh" }}>
      <section style={{ borderBottom: "1px solid #e4ddd0", background: "linear-gradient(180deg, #fcfaf6, #ffffff)" }}>
        <Container style={{ padding: "54px 32px 46px" }}>
          ...
          <p style={{ margin: "16px 0 0", maxWidth: 600, fontSize: 14.5, lineHeight: 1.7, color: "rgba(20,18,14,0.74)" }}>
          ...
          <div className="kb-steps-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginTop: 34 }}>
            {STEPS.map((s) => (
              <div key={s.n} style={{ border: "1px solid #e4ddd0", padding: "18px 20px", background: "#f7f4ee" }}>
                ...
                <p style={{ margin: "8px 0 0", fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>{s.body}</p>
              </div>
            ))}
          </div>
        </Container>
      ...
      <Container style={{ padding: "44px 32px 90px" }}>
        {hasActive && (
          <div style={{ border: "1px solid rgba(184,135,60,0.38)", background: "#f7f4ee", padding: "16px 20px", marginBottom: 30, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        ...
        <div className="kb-subscribe-grid" style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 40, alignItems: "start" }}>
          <div>
            {/* Step 1: format */}
            <div style={{ ...micro, color: GOLD }}>01 · Your format</div>
            <div className="kb-subformats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginTop: 14 }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      background: "#f7f4ee",
                      border: `1px solid ${active ? GOLD : "#e4ddd0"}`,
                  ...
                  >
                    <div style={{ height: 80, display: "grid", placeItems: "center" }}>
                      <FormatGlyph formatKey={key} liquid={frag?.liquid ?? "#d7c3ad"} height={72} />
                    </div>
            ...
            <div className="kb-submode-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 }}>
              {(
                [
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    style={{ textAlign: "left", cursor: "pointer", background: "#f7f4ee", border: `1px solid ${active ? GOLD : "#e4ddd0"}`, padding: "16px 18px", color: CREAM, display: "grid", gap: 6 }}
                  >
                    <div style={{ fontFamily: SERIF, fontSize: 22, lineHeight: 1.05 }}>{o.title}</div>
                    <div style={{ fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>{o.body}</div>
                  </button>
                );
              })}
            </div>
            ...
            <div className="kb-subscents-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginTop: 16 }}>
              {shown.map((f) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    style={{ textAlign: "left", cursor: "pointer", background: "#f7f4ee", border: `1px solid ${active ? GOLD : "#e4ddd0"}`, padding: 0, color: CREAM, display: "grid" }}
                  >
                    ...
                    <div style={{ padding: "12px 14px 14px", display: "grid", gap: 6 }}>
                      ...
                      <div style={{ ...micro, fontSize: 8.5, color: "rgba(20,18,14,0.68)" }}>{profileOf(f).join(" · ")}</div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                        <span style={{ fontFamily: MONO, fontSize: 10.5, color: GOLD }}>{moneyExact(subscriptionPrice(f, format))}/mo</span>
                        <span style={{ fontFamily: MONO, fontSize: 9.5, color: "rgba(20,18,14,0.68)", textDecoration: "line-through" }}>{money(formatPrice(f, format))}</span>
                      </div>
                    </div>
                  </button>
                );
          ...
          {/* Summary */}
          <aside className="kb-subscribe-summary" style={{ position: "sticky", top: 90, border: "1px solid rgba(184,135,60,0.34)", background: "#f7f4ee", padding: 24, display: "grid", gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: MONO, fontSize: 10.5, color: "rgba(20,18,14,0.68)", marginTop: 6 }}>Every month for {SUBSCRIPTION_MONTHS} months</div>
            </div>
            <div style={{ borderTop: "1px solid #e4ddd0", paddingTop: 16 }}>
              {mode === "surprise" ? (
                <div style={{ display: "grid", gap: 6 }}>
                  ...
                  <div style={{ fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>A random scent from the collection, never repeated. Revealed when it ships.</div>
                </div>
              ) : frag ? (
                ...
                <div style={{ fontSize: 13, color: "rgba(20,18,14,0.68)", lineHeight: 1.6 }}>Pick this month's scent below to see your price, or let the house choose.</div>
              )}
            </div>
            <div style={{ borderTop: "1px solid #e4ddd0", paddingTop: 16, display: "grid", gap: 8, fontFamily: MONO, fontSize: 11.5 }}>
              {mode === "surprise" ? (
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", color: "rgba(20,18,14,0.68)" }}>
                  ...
                  <div style={{ fontSize: 10.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)", fontFamily: "inherit" }}>
            ...
            {error && <div style={{ fontSize: 11.5, color: "#a8431b", lineHeight: 1.5 }}>{error}</div>}
            <button
              className="kb-cta"
            ...
            <div style={{ fontSize: 10.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)", textAlign: "center" }}>
