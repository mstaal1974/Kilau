...
const serif: CSSProperties = { fontFamily: "'Cormorant Garamond',serif", color: "#14120e" };
...
export default function ConceiveFragrance({ fragrances, busy, onAdd, onRefine, onClose }: ConceiveFragranceProps) {
  ...
  const liquid = conception?.liquid ?? "#e3d8c9";

  return (
    <section
      aria-label="Conceive a fragrance with AI"
      style={{ border: "1px solid rgba(184,135,60,0.26)", background: "rgba(244,240,232,0.5)", padding: 24 }}
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)", maxWidth: 620 }}>
            Type the fragrance the atelier sourced. Claude proposes a Kilau Bali name for its profile, writes the
      ...
      {error && (
        <p role="alert" style={{ margin: "14px 0 0", fontSize: 12.5, color: "#a8431b", lineHeight: 1.6 }}>
          {error}
        </p>
      )}

      {status === "thinking" && !conception && (
        <p aria-live="polite" style={{ margin: "18px 0 0", ...serif, fontStyle: "italic", fontSize: 18, color: "rgba(20,18,14,0.74)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            ...
            <div style={panel}>
              ...
              <input
                ...
                style={{ ...field, ...serif, fontSize: 30, height: 52, border: 0, borderBottom: `1px solid ${nameTaken ? "#f0d3c5" : "rgba(184,135,60,0.3)"}`, padding: "0 2px" }}
                value={chosenName}
                onChange={(e) => setChosenName(e.target.value)}
              />
              {nameTaken && <div style={{ fontSize: 11, color: "#a8431b" }}>Already in the catalogue — pick an alternate.</div>}
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                {conception.profile.map((p) => (
                  <span key={p} style={{ ...label, color: "rgba(20,18,14,0.82)", fontSize: 10 }}>
                    {p}
                  </span>
                )).flatMap((el, i) => (i ? [<span key={`dot-${i}`} style={{ color: GOLD, opacity: 0.6 }}>·</span>, el] : [el]))}
              </div>
              <div style={{ fontSize: 12.5, color: "rgba(20,18,14,0.74)", lineHeight: 1.6 }}>{conception.inspiration}</div>
                    ...
                    {[conception.name, ...conception.alternates].map((n) => (
                      <button
                        ...
                        style={{
                          ...chip,
                          cursor: "pointer",
                          borderColor: n === chosenName ? GOLD : "rgba(184,135,60,0.26)",
                          color: n === chosenName ? GOLD : "#14120e",
                        }}
                      >
                        {n}
                      </button>
            ...
            <div style={panel}>
              ...
              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.65, color: "rgba(20,18,14,0.82)" }}>{conception.story}</p>
              <div style={{ ...label, fontSize: 8 }}>Packaging copy</div>
              {conception.copy.split(/\n\s*\n/).map((para, i) => (
                <p key={i} style={{ margin: 0, fontSize: 12.5, lineHeight: 1.7, color: "rgba(20,18,14,0.74)" }}>
                  {para}
                </p>
              ))}
            </div>
            ...
            <div style={panel}>
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 4 }}>
                <span title={`Liquid ${conception.liquid}`} style={{ width: 22, height: 22, background: conception.liquid, border: "1px solid #e4ddd0" }} />
                <span title={`Accent ${conception.accent}`} style={{ width: 22, height: 22, background: conception.accent, border: "1px solid #e4ddd0" }} />
          ...
          <div style={{ ...panel, flexDirection: "row", flexWrap: "wrap", gap: 20, alignItems: "stretch" }}>
            <div
              ...
              style={{
                ...
                border: `1px ${dragging ? "solid" : "dashed"} ${dragging ? GOLD : "rgba(184,135,60,0.26)"}`,
                display: "grid",
                placeItems: "center",
                position: "relative",
                overflow: "hidden",
            ...
            <div style={{ flex: 1, minWidth: 240, display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <p style={{ margin: 0, fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>
                Drop a <strong style={{ color: "#14120e", fontWeight: 500 }}>PNG, JPG or WebP</strong> of the bottle (up to 4 MB). A
              ...
              {imageInfo && !imageInfo.ok && <div style={{ fontSize: 12, color: "#a8431b" }}>{imageInfo.reason}</div>}
              {imageInfo?.ok && (
                <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: imageInfo.transparent || imageInfo.format === "jpeg" ? "#2f6b3e" : "#a8431b" }}>
