...
export default function StaffDesk({ isAdmin = false, onSignOut }: { isAdmin?: boolean; onSignOut: () => void }) {
  ...
  if (!orders) {
    ...
    if (isSupabaseConfigured) {
      return <main style={{padding: 32, color: "#14120e"}}><h1>Staff order desk</h1><p>{busy ? "Loading orders…" : error || "Checking staff access…"}</p><p>Your individual account must be authorised for order fulfilment.</p><button onClick={() => void refresh("account-session")}>Retry</button><a href="#/">Return to shop</a></main>;
    }
...
function Gate({
  ...
  return (
    <main
      ...
      style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 24, background: "#ffffff" }}
    >
      <form onSubmit={onSubmit} style={{ width: "100%", maxWidth: 360 }}>
        <div style={{ ...label, color: "rgba(138,98,21,1)", letterSpacing: "0.28em" }}>Kilau Bali</div>
        <h1 style={{ margin: "12px 0 0", fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 40, color: "#14120e" }}>
          Order desk
        </h1>
        <p style={{ margin: "10px 0 22px", fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>
        ...
        {error && (
          <p role="alert" style={{ margin: "14px 0 0", fontSize: 12, color: "#a32e24" }}>
            {error}
          </p>
        )}
      </form>
...
function AdminWait({ error, onRetry, busy }: { error: string | null; onRetry: () => void; busy: boolean }) {
  return (
    <main
      ...
      style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 24, background: "#ffffff", textAlign: "center" }}
    >
      <div>
        <div style={{ ...label, color: "rgba(138,98,21,1)", letterSpacing: "0.28em" }}>Kilau Bali · Order desk</div>
        <p style={{ margin: "16px 0 0", fontSize: 13, color: error ? "#a32e24" : "rgba(20,18,14,0.68)" }}>
...
function Desk({
  ...
  return (
    <main className="kb-screen" data-screen-label="Staff desk" style={{ minHeight: "100vh", background: "#ffffff", padding: "34px 26px 80px" }}>
      <div style={{ maxWidth: 1500, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
          <div>
            <div style={{ ...label, color: "rgba(138,98,21,1)", letterSpacing: "0.28em" }}>Kilau Bali · Order desk</div>
            <h1 style={{ margin: "10px 0 0", fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontSize: 42, color: "#14120e" }}>
              {toPack} to pack
              {awaiting > 0 && <span style={{ fontSize: 20, color: "rgba(20,18,14,0.68)" }}> · {awaiting} awaiting tracking</span>}
            </h1>
          </div>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", margin: "22px 0 16px" }}>
          {FILTERS.map((f) => (
            <button
              ...
              style={{
                ...
                height: 32,
                padding: "0 14px",
                borderColor: filter === f.key ? GOLD : "#e4ddd0",
                color: filter === f.key ? GOLD : "rgba(20,18,14,0.74)",
              }}
            >
              {f.label}
            </button>
          ...
          {!returnLines(returnAddress).length && (
            <span style={{ ...label, color: "rgba(163,46,36,0.9)" }}>Labels will print without a sender</span>
          )}
          {!isSupabaseConfigured && <span style={{ ...label, color: "rgba(163,46,36,0.9)" }}>Demo data</span>}
        </div>

        {(failed || error) && (
          <p role="alert" style={{ margin: "0 0 14px", fontSize: 12, color: "#a32e24" }}>
            {failed ?? error}
          </p>
        )}

        {shown.length === 0 ? (
          <p style={{ fontSize: 13, color: "rgba(20,18,14,0.68)" }}>Nothing here. Try another filter.</p>
        ) : (
...
function OrderRow({
  ...
  return (
    <article
      className="kb-desk-row"
      style={{
        display: "grid",
        gridTemplateColumns: "44px 108px minmax(220px, 1.4fr) 96px minmax(170px, 1fr) minmax(190px, 1fr) 200px",
        gap: 14,
        alignItems: "start",
        border: `1px solid ${order.packed ? "rgba(184,135,60,0.22)" : "#e4ddd0"}`,
        background: order.packed ? "rgba(200,160,99,0.04)" : "rgba(20,18,14,0.01)",
        padding: "14px 16px",
      }}
    >
      ...
      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "rgba(20,18,14,0.74)", lineHeight: 1.6 }}>
        {placed.toLocaleDateString("en-AU", { day: "2-digit", month: "short" })}
        <br />
        <span style={{ color: "rgba(20,18,14,0.62)" }}>{placed.toLocaleTimeString("en-AU", { hour: "2-digit", minute: "2-digit" })}</span>
      </div>
      ...
      <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 12, color: "rgba(20,18,14,0.82)" }}>{moneyExact(order.amount_cents)}</div>

      <div style={{ fontSize: 12, color: "rgba(20,18,14,0.82)", lineHeight: 1.55, wordBreak: "break-word" }}>
        {order.ship_name && <div>{order.ship_name}</div>}
        {order.email && <div style={{ fontSize: 11, color: "rgba(20,18,14,0.68)" }}>{order.email}</div>}
        {order.ship_phone && <div style={{ fontSize: 11, color: "rgba(20,18,14,0.68)" }}>{order.ship_phone}</div>}
      </div>

      <div style={{ fontSize: 12, color: "rgba(20,18,14,0.74)", lineHeight: 1.5 }}>
        <div style={{ ...label, fontSize: 8, color: order.delivery_method === "alternate" ? "rgba(163,46,36,0.9)" : "rgba(20,18,14,0.68)" }}>
          {order.delivery_method === "alternate" ? "Alternate delivery" : "Australia Post"}
        </div>
        {lines.length ? lines.map((l, n) => <div key={n}>{l}</div>) : <span style={{ color: "rgba(20,18,14,0.62)" }}>No address on the order</span>}
        {order.ship_notes && <div style={{ marginTop: 4, fontSize: 11, color: "rgba(138,98,21,1)" }}>{order.ship_notes}</div>}
      </div>
      ...
      <div style={{ display: "grid", gap: 6 }}>
        ...
        {order.tracking_number && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{ ...label, fontSize: 8.5, color: "rgba(138,98,21,1)", textDecoration: "none" }}
          >
            Track this parcel →
          </a>
        )}
        <span style={{ ...label, fontSize: 8, color: "rgba(20,18,14,0.62)", wordBreak: "break-all" }}>{order.order_ref}</span>
      </div>
    </article>
  );
}
...
function PackList({ orders }: { orders: StaffOrder[] }) {
  return (
    <section className="kb-print kb-print-list" aria-hidden>
      <h1 className="kb-print-title">
        Kilau Bali — pack list
        <span>
          {new Date().toLocaleString("en-AU", { dateStyle: "medium", timeStyle: "short" })} · {orders.length} {orders.length === 1 ? "order" : "orders"}
        </span>
      </h1>
      {orders.map((o) => (
        <article key={o.order_ref} className="kb-pack">
          <header>
...
function Labels({ orders, from }: { orders: StaffOrder[]; from: string }) {
  ...
  return (
    <section className="kb-print kb-print-labels" aria-hidden>
      {orders.map((o) => (
        <article key={o.order_ref} className="kb-label">
          {sender.length > 0 && (
            <div className="from">
              {sender.map((l, n) => (
                <div key={n}>{l}</div>
