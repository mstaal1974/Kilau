...
export default function About({ vip, signedIn, onJoin }: AboutProps) {
  const RANGES = [
    ...
    ["Find your Kilau", "Match", "Tell us what you love; we suggest yours."],
  ];
  return (
    <main data-screen-label="About">
      <Container style={{ padding: "48px 32px 20px" }}>
        ...
        <p style={{ ...body, margin: "14px 0 0", maxWidth: 620 }}>Discover it. Wear it. Drive with it. Live in it. Kilau Bali is a batch atelier: each fragrance is poured in small numbers and offered in every format your day needs.</p>
        <div style={{ marginTop: 30, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
          {RANGES.map(([name, fmt, copy]) => (
            <div key={name} style={{ border: "1px solid #e4ddd0", padding: 16 }}>
              ...
              <p style={{ margin: "8px 0 0", fontSize: 12.5, lineHeight: 1.55, color: "rgba(20,18,14,0.74)" }}>{copy}</p>
            </div>
          ))}
        </div>
      </Container>
