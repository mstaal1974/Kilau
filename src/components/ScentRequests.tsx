    ...
    <section>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "rgba(20,18,14,0.68)" }}>
      ...
      <div style={{ marginTop: 22, border: "1px solid #e4ddd0" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 90px 1fr 150px 200px", gap: 14, padding: "10px 16px", borderBottom: "1px solid #e4ddd0" }}>
        ...
        {loading && !shown.length ? (
          ...
          <div style={{ padding: 22, fontSize: 12, color: "rgba(20,18,14,0.68)" }}>
            {filter === "open" ? "No open requests. When a search finds nothing in the house, the customer's ask lands here." : "Nothing here yet."}
          </div>
        ) : (
          shown.map((g) => (
            <div key={g.key} style={{ display: "grid", gridTemplateColumns: "1fr 90px 1fr 150px 200px", gap: 14, alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #e4ddd0" }}>
              <div>
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: "#14120e" }}>{g.query}</div>
                <span style={{ ...chip, marginTop: 4, display: "inline-block", color: g.status === "open" ? GOLD : "rgba(20,18,14,0.68)" }}>{g.status}</span>
              </div>
              <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 14, color: GOLD }}>×{g.count}</div>
              <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10.5, color: "rgba(20,18,14,0.74)", lineHeight: 1.6, wordBreak: "break-all" }}>
                {g.emails.length ? g.emails.join(", ") : <span style={{ color: "rgba(20,18,14,0.62)" }}>no email left</span>}
              </div>
