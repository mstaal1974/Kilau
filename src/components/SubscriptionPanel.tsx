...
import { type Fragrance, GOLD, CREAM, money, moneyExact, GOLD_LEAF } from "../lib/data";
...
const STATUS: Record<Subscription["status"], { label: string; color: string }> = {
  active: { label: "Active", color: "#2f6b3e" },
  cancelled: { label: "Cancelled", color: "rgba(20,18,14,0.68)" },
...
export default function SubscriptionPanel({ subscriptions, fragrances, loading, onChanged }: SubscriptionPanelProps) {
  ...
  return (
    <section style={{ marginTop: 44 }}>
      ...
      {loading ? (
        <p style={{ marginTop: 16, fontSize: 13, color: "rgba(20,18,14,0.68)" }}>Loading your subscription…</p>
      ) : subscriptions.length === 0 ? (
        <div style={{ marginTop: 20, border: "1px solid #e4ddd0", padding: "34px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontFamily: SERIF, fontSize: 24, color: "rgba(20,18,14,0.82)" }}>A new scent every month, 10% off.</div>
            <p style={{ margin: "8px 0 0", fontSize: 12.5, color: "rgba(20,18,14,0.68)", maxWidth: 520, lineHeight: 1.6 }}>
          ...
          <button className="kb-ghost" style={btnGhost} onClick={() => navigate(paths.subscribe())}>Explore the Monthly Pour <Arrow size={10} /></button>
        </div>
      ) : (
        <div style={{ marginTop: 20, display: "grid", gap: 16 }}>
          {subscriptions.map((s) => {
            ...
            return (
              <article key={s.id} style={{ border: `1px solid ${s.status === "active" ? "rgba(184,135,60,0.38)" : "#e4ddd0"}`, background: "#f7f4ee", padding: 24 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                  <div>
                    <div style={{ fontFamily: SERIF, fontSize: 28, color: CREAM, lineHeight: 1 }}>{subscriptionLabel(s)}</div>
                    <div style={{ marginTop: 8, fontFamily: MONO, fontSize: 11, color: "rgba(20,18,14,0.74)" }}>
                ...
                <div style={{ marginTop: 18, height: 2, background: "#e4ddd0" }}>
                  <div style={{ height: "100%", width: `${Math.round((monthsBilled(s) / s.months) * 100)}%`, background: GOLD_LEAF }} />
                </div>

                {s.status === "active" && n !== null && (
                  <div className="kb-subnext-grid" style={{ marginTop: 22, display: "grid", gridTemplateColumns: "88px 1fr auto", gap: 18, alignItems: "center", borderTop: "1px solid #e4ddd0", paddingTop: 20 }}>
                    {next ? (
                      ...
                      <div style={{ height: 104, border: `1px dashed ${surprise ? "rgba(184,135,60,0.38)" : "#e4ddd0"}`, display: "grid", placeItems: "center", fontFamily: SERIF, fontSize: 30, color: GOLD }}>
                        {surprise ? "?" : ""}
                      </div>
                    )}
                    <div style={{ display: "grid", gap: 6, minWidth: 0 }}>
                      ...
                      {surprise && (
                        <div style={{ fontSize: 12.5, lineHeight: 1.6, color: "rgba(20,18,14,0.74)" }}>
                      ...
                      {next && (
                        <div style={{ fontFamily: MONO, fontSize: 11, color: "rgba(20,18,14,0.74)" }}>
                      ...
                      {!surprise && (
                        <label style={{ display: "grid", gap: 6 }}>
                          ...
                          <select
                            ...
                            style={{ background: "#ffffff", color: CREAM, border: "1px solid #e4ddd0", height: 40, padding: "0 12px", fontFamily: MONO, fontSize: 11.5, minWidth: 220 }}
                          >
                ...
                <div style={{ marginTop: 22, borderTop: "1px solid #e4ddd0", paddingTop: 16 }}>
                    ...
                    {s.deliveries.map((d) => {
                      ...
                      return (
                        <div key={d.id} style={{ display: "grid", gridTemplateColumns: "70px 1fr auto auto", gap: 14, alignItems: "center", fontFamily: MONO, fontSize: 11 }}>
                          <span style={{ color: "rgba(20,18,14,0.68)" }}>Month {d.month}</span>
                          ...
                          <span style={{ color: d.status === "delivered" ? "#2f6b3e" : "rgba(20,18,14,0.74)" }}>{DELIVERY[d.status]} · {fmtDate(d.billedAt)}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                {s.status === "active" && (
                  <div style={{ marginTop: 20, display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
                    {confirmCancel === s.id ? (
                      <>
                        <span style={{ fontSize: 12.5, color: "rgba(20,18,14,0.74)" }}>End the subscription? Months already paid still ship.</span>
                        <button className="kb-ghost" style={{ ...btnGhost, height: 36, color: "#a8431b", borderColor: "#f0d3c5" }} disabled={busy} onClick={() => void cancel(s)}>Yes, cancel</button>
                      ...
                      <>
                        ...
                        <button style={{ ...btnLink, color: "rgba(20,18,14,0.68)" }} onClick={() => setConfirmCancel(s.id)}>Cancel subscription</button>
                      </>
                    )}
                  </div>
                )}
