    ...
    <section>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: "6px 0 0", fontSize: 12, color: "rgba(20,18,14,0.68)", maxWidth: 760, lineHeight: 1.6 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {(["active", "completed", "cancelled", "all"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} style={{ ...chip, cursor: "pointer", borderColor: filter === f ? GOLD : "#e4ddd0", color: filter === f ? GOLD : "rgba(20,18,14,0.74)" }}>
              {f}
            </button>
          ))}
        </div>
      ...
      <div style={{ marginTop: 22, display: "grid", gap: 12 }}>
        {loading && !shown.length ? (
          <div style={{ border: "1px solid #e4ddd0", padding: 22, fontSize: 12, color: "rgba(20,18,14,0.68)" }}>Loading…</div>
        ) : !shown.length ? (
          <div style={{ border: "1px solid #e4ddd0", padding: 22, fontSize: 12, color: "rgba(20,18,14,0.68)" }}>No subscriptions here yet.</div>
        ) : (
          shown.map((s) => {
            ...
            return (
              <div key={s.id} style={{ border: "1px solid #e4ddd0", background: "#f7f4ee", padding: "16px 18px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 1.2fr 150px 220px", gap: 14, alignItems: "center" }}>
                  <div>
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: "#14120e" }}>{FORMAT_BY_KEY[s.format].name}</div>
                    <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10.5, color: "rgba(20,18,14,0.74)", wordBreak: "break-all" }}>{s.userEmail ?? "—"}</div>
                  </div>
                  <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 11, color: "rgba(20,18,14,0.74)" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 19, color: "#14120e" }}>{surprise ? "Surprise — house draws" : (next?.name ?? "—")}</div>
                    {surprise ? (
                  ...
                  <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 10.5, color: "rgba(20,18,14,0.74)" }}>
                    <div style={label}>Bills</div>
                    {billing ? fmtDate(billing) : "—"}
                  </div>
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
                    {s.status === "active" && n !== null && s.stripeSubscriptionId && (
                      <span style={{ ...label, alignSelf: "center", color: "#2f6b3e" }}>Stripe bills month {n}</span>
                    )}
                    ...
                    {s.status === "active" && (
                      <button style={{ ...btnGhost, height: 32, padding: "0 12px", color: "#a8431b" }} disabled={busy === s.id} onClick={() => void cancel(s)}>
                        Cancel
                      </button>
                    )}
                  </div>
                ...
                {s.deliveries.length > 0 && (
                  <div style={{ marginTop: 12, borderTop: "1px solid #e4ddd0", paddingTop: 10, display: "grid", gap: 6 }}>
                    {s.deliveries.map((d) => {
                      ...
                      return (
                        <div key={d.id} style={{ display: "grid", gridTemplateColumns: "80px 1fr 90px 130px auto", gap: 12, alignItems: "center", fontFamily: "'Space Mono',monospace", fontSize: 10.5 }}>
                          ...
                          <span style={{ color: "#14120e" }}>{f?.name ?? d.fragranceId}</span>
                          <span style={{ color: GOLD }}>{money(d.chargeCents)}</span>
                          <span style={{ color: "rgba(20,18,14,0.74)" }}>{d.status} · {fmtDate(d.billedAt)}</span>
