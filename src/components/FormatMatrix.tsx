...
const STATUS_COLOR: Record<FormatStatus, string> = { live: "#cfe4ce", coming_soon: "#8a6215", hidden: "rgba(20,18,14,0.62)" };
...
const cellStyle: CSSProperties = { padding: "8px 6px", borderBottom: "1px solid #e4ddd0", verticalAlign: "top", minWidth: 96 };
...
export default function FormatMatrix({ fragrances, configured, onReload }: FormatMatrixProps) {
  ...
  const select = (f: Fragrance, k: FormatKey) => (
    <select
      ...
      style={{ ...field, appearance: "auto", height: 26, fontSize: 9.5, padding: "0 4px", color: STATUS_COLOR[statusOf(f, k)], borderColor: "#e4ddd0" }}
    >
  ...
  return (
    <div>
      {/* Bulk actions */}
      <div style={{ border: "1px solid rgba(184,135,60,0.26)", background: "rgba(244,240,232,0.5)", padding: 16, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <span style={{ ...label, color: "rgba(138,98,21,1)", marginRight: 6 }}>Bulk actions</span>
      ...
      {note && (
        <p role="status" style={{ margin: "12px 0 0", fontFamily: "'Space Mono',monospace", fontSize: 11, color: "#8a6215" }}>{note}</p>
      )}
      ...
      <div style={{ overflowX: "auto", marginTop: 18 }}>
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 980 }}>
              ...
              {FORMATS.map((d) => {
                ...
                return (
                  <th key={d.key} style={{ ...cellStyle, ...label, textAlign: "left" }}>
                    {d.label}
                    <div style={{ marginTop: 3, fontSize: 8, color: "rgba(20,18,14,0.62)", letterSpacing: "0.1em", textTransform: "none" }}>{t.live} live · {t.stock} units</div>
                  </th>
                );
              })}
              <th style={cellStyle} />
          ...
          <tbody>
            {fragrances.map((f) => (
              <tr key={f.id} style={{ background: dirty(f.id) ? "rgba(200,160,99,0.04)" : "none" }}>
                <td style={cellStyle}>
                  <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 19, color: "#14120e", lineHeight: 1 }}>{f.name}</div>
                  ...
                  return (
                    <td key={k} style={cellStyle}>
                      <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                        {derived ? (
                          <span title="Derived: min of 50ml, wash, moisturiser" style={{ ...field, height: 26, width: 48, display: "grid", placeItems: "center", fontSize: 11, color: "rgba(20,18,14,0.74)", borderStyle: "dashed" }}>
                          ...
                          <input
                            ...
                            style={{ ...field, height: 26, width: 52, padding: "0 6px", fontSize: 11, color: stockOf(f, k) <= (f.lowStock ?? 5) ? "#a8431b" : "#14120e" }}
                          />
                        )}
                      ...
                      <div style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}>
                        <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, color: "rgba(20,18,14,0.68)" }}>$</span>
                        {derived ? (
                          <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: "rgba(20,18,14,0.74)" }}>{money(priceOf(f, k)).slice(1)} <s style={{ opacity: 0.5 }}>{money(skuOf(f, k).compareAt ?? 0).slice(1)}</s></span>
                        ) : (
                          <input
                            type="number"
                            min={0}
      ...
      <p style={{ margin: "14px 0 0", fontSize: 11.5, lineHeight: 1.6, color: "rgba(20,18,14,0.68)" }}>
