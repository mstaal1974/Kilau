...
export default function QuickView({ frag, initialFormat, onClose, onAdd }: QuickViewProps) {
    ...
    return (
      <button
        ...
        style={{
          ...
          width: "100%",
          textAlign: "left",
          background: active ? "rgba(200,160,99,0.1)" : "none",
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ ...micro, fontSize: 8, display: "block", marginTop: 3, color: soon ? GOLD : "rgba(20,18,14,0.68)" }}>
            {soon ? (notified.has(s.key) ? "We'll let you know" : "Coming soon · Notify me") : s.availability}
          </span>
          </span>
        </span>
        <span style={{ fontFamily: MONO, fontSize: 12, whiteSpace: "nowrap", color: soon ? "rgba(20,18,14,0.68)" : CREAM }}>
          {s.compareAt && <s style={{ marginRight: 8, color: "rgba(20,18,14,0.68)" }}>{money(s.compareAt)}</s>}
          {money(s.price)}
        </span>
      </button>
    );
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`${frag.name} — choose a format`} style={{ position: "fixed", inset: 0, zIndex: 90, display: "flex", justifyContent: "flex-end" }}>
      ...
      <aside className="kb-drawer kb-scroll" style={{ position: "relative", width: 460, maxWidth: "100%", height: "100%", overflowY: "auto", background: "#ffffff", borderLeft: "1px solid #e4ddd0", padding: 28, display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        <div style={{ fontFamily: SERIF, fontStyle: "italic", fontSize: 20, color: "rgba(20,18,14,0.9)" }}>How would you like it?</div>
        ...
        <div style={{ marginTop: "auto", paddingTop: 8, borderTop: "1px solid #e4ddd0", display: "grid", gap: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: SERIF, fontSize: 16, color: "rgba(20,18,14,0.9)" }}>{chosen.def.name}</span>
            <span style={{ fontFamily: MONO, fontSize: 15, color: CREAM }}>{money(chosen.price)}</span>
          </div>
          <button className="kb-cta" style={{ ...btnGold, justifyContent: "center", opacity: chosen.buyable ? 1 : 0.5 }} disabled={!chosen.buyable} onClick={() => onAdd(frag, key, 1)}>
