...
export default function BottleImage({
  ...
  if (photo) {
    ...
    return (
      <div style={{ position: "relative", width: "100%", height, overflow: "hidden", background: "#faf8f3", ...style }}>
        <img
          src={imageUrl}
          alt={alt}
          loading="lazy"
  ...
  if (!imageUrl || failed === imageUrl) {
    ...
    return (
      <div style={{ position: "relative", width: "100%", height, overflow: "hidden", background: "#faf8f3", ...style }}>
        ...
        />
        <div aria-hidden style={{ position: "absolute", inset: 0, background: "radial-gradient(75% 70% at 50% 45%, transparent 35%, rgba(255,255,255,0.8) 100%)" }} />
      </div>
    );
  }
  return (
