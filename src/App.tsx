...
import ChooseKilau from "./components/ChooseKilau";
...
export default function App() {
  ...
  if (route.view === "scent") {
    return (
      <div className="kb-grain" style={{ minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
        <ScentDna
          fragrances={fragrances}
    ...
    <div className="kb-grain" style={{ minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
      ...
      {route.view === "account" && (
        <MyOrders
          ...
          notice={
            stripeNotice ? (
              <div style={{ marginTop: 24, border: "1px solid rgba(184,135,60,0.45)", background: "#f7f4ee", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 22, color: "#14120e" }}>{stripeNotice.kind === "subscription" ? "You're in." : "Thank you — your order is confirmed."}</span>
                <span style={{ fontSize: 13, color: "rgba(20,18,14,0.74)" }}>{stripeNotice.detail}</span>
              </div>
            ) : null
          }
        />
