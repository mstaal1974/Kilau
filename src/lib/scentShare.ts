...
//   • mint a short share code (kilaubali.com/scent/7HD92K) when Supabase
...
export function scentUrl(code: string): string {
  const origin = typeof window === "undefined" ? "https://kilaubali.com" : window.location.origin;
...
export function shareTargets(url: string, text: string): ShareTarget[] {
  ...
  return [
    ...
    { id: "email", label: "Email", href: `mailto:?subject=${encodeURIComponent("My Scent DNA — Kilau Bali")}&body=${both}` },
  ];
}
